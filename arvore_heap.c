...
void trocar (NO** V, int Pai , int i){
    ...
    Valor_pai -> codigo_aposta = (char*) calloc(33,sizeof(char));
    Valor_atual -> codigo_aposta = (char*) calloc(33,sizeof(char));
...
void extrair_MAX(NO** V, int indice_apostas, NO** Ganhadores_max, int indice_ganhadores ){
    ...
    Ganhadores_max[indice_ganhadores]->codigo_aposta= (char*) calloc(33,sizeof(char));
    strcpy((Ganhadores_max[indice_ganhadores]->codigo_aposta),(V[0]->codigo_aposta));
    Ganhadores_max[indice_ganhadores]->acertos=V[0]->acertos;
    trocar(V,0,indice_apostas);
    V[indice_apostas]='\0';
    indice_apostas-=1;
...
void extrair_MIN(NO** V, int indice_apostas, NO** Ganhadores_min, int indice_ganhadores ){
    ...
    Ganhadores_min[indice_ganhadores]->codigo_aposta= (char*) calloc(33,sizeof(char));
    strcpy((Ganhadores_min[indice_ganhadores]->codigo_aposta),(V[0]->codigo_aposta));
    Ganhadores_min[indice_ganhadores]->acertos=V[0]->acertos;
    printf("%s \n",V[0]->codigo_aposta);
    trocar(V,0,indice_apostas);
    V[indice_apostas]='\0';
    indice_apostas-=1;
...
int main(int argc, char* argv[]){
  ...
  char* entrada = (char*)calloc(33,sizeof(char));
  ...
  while(fscanf(input, "%s", entrada)==1){
    ...
    Vetor_MAX[indice]->codigo_aposta= (char*) calloc(33,sizeof(char));
    Vetor_MIN[indice]->codigo_aposta= (char*) calloc(33,sizeof(char));
    ...
    if(num_maior==pontuacao){maior+=1;}
      ...
      if(num_maior<pontuacao){maior=1;
    num_maior=pontuacao;}}
    if(num_menor==pontuacao){menor+=1;}
    else{if(num_menor>pontuacao){menor=1;
    num_menor=pontuacao;}}
    
    //atribuindo pontuacao a apostas
    ...
    int ultimo_no_interno = ((qt_apostadores -1)-1)/2;
